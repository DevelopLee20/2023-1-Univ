...
public class CGPanel extends JPanel {
    ...
    class xypos{
        int x, y;
        boolean status;
        public xypos(int x, int y, boolean status){
            this.x = x;
            this.y = y;
            this.status = status;
        }
    ...
    class panel_control extends JPanel{
        ...
        panel_control(){
            ...
            b_x1 = new JTextField();  //좌표값 입력 텍스트필드
            b_y1 = new JTextField();
            b_x2 = new JTextField();
            b_y2 = new JTextField();
            b_btn1 = new JButton("BSH");
            b_btnClear = new JButton("Clear");
            ...
            b_btn1.addActionListener(e -> {
                int_x1 = Integer.parseInt(b_x1.getText());
                int_y1 = Integer.parseInt(b_y1.getText());
                int_x2 = Integer.parseInt(b_x2.getText());
                int_y2 = Integer.parseInt(b_y2.getText());
                new Graphics_Alg().BSH_Alg(int_x1, int_y1, int_x2, int_y2);

                gpanel.repaint();
            });
            b_btnClear.addActionListener(e -> {
                arr.clear();
                gpanel.repaint();
                b_x1.setText("");
                b_x2.setText("");
        ...
        class Graphics_Alg{
            ...
            public int b_x1, b_y1, b_x2, b_y2;
            public int p0;
            private float m;
            ...
            public void BSH_Alg(int x1, int y1, int x2, int y2){
                this.x1 = x1;
                this.y1 = y1;
                this.x2 = x2;
                this.y2 = y2;
                ...
                if(m_bool){
                    ...
                    for(int x=this.x1; x<=this.x2; x++){
                        arr.add(0, new xypos(x, y, stat));
                        stat = false;
                        if(this.p0 >= 0){
                            this.p0 = this.p0 + 2 * (dy * dx);
                        }
                        else{
                            this.p0 = this.p0 + 2 * dy;
                            stat = true;
                        }
            ...
            public void DDA_Alg(int x1, int y1, int x2, int y2){
                ...
                if(m_bool){
                    float y = this.y1;
                    for(int x=this.x1; x<=this.x2; x++){
                        arr.add(0, new xypos(x, return_int(y), stat));
                        y += m;
                    }
                }
                else{
                    float x = this.x1;
                    for(int y=this.y1; y<=this.y2; y++){
                        arr.add(0, new xypos(return_int(x), y, stat));
                        x += 1/m;
                    }
                }
            }
