        ...
        public void paintComponent(Graphics g){     // g는 GraphicsContext 객체
            ...
            if(1>m){
                for(int x=(int)start_x; x<(int)end_x; x++){
                    if(start_y % 1 > 0.5){
                        g2.fillRect((int)x*pix, -(int)(start_y+1)*pix, pix, pix);
                    }
                    else{
                        g2.fillRect((int)x*pix, -(int)(start_y)*pix, pix, pix);
                    }
                    start_y = start_y + m;
                }
            }
            else{
                for(int y=(int)start_y; y<(int)start_y; y++){
                    if(start_x % 1 > 0.5){
                        g2.fillRect((int)start_x*pix+1, (int)y, pix, pix);
                    }
                    else{
                        g2.fillRect((int)start_x*pix, (int)y, pix, pix);
                    }
                }
            }
    ...
    class panel_control extends JPanel{
        ...
        panel_control(){
            ...
            btn1.addActionListener(e -> {
                // DDA_Alg
                int DDA_x1 = Integer.parseInt(x1.getText());
                int DDA_y1 = Integer.parseInt(y1.getText());
                int DDA_x2 = Integer.parseInt(x2.getText());
                int DDA_y2 = Integer.parseInt(y2.getText());
                start_x = DDA_x1;
                start_y = DDA_y1;
                end_x = DDA_x2;
                end_y = DDA_y2;
